...
import { removeUser, updateUser, addUser } from "../store/roleSlice";
...
function RoleHeader(props) {
  ...
  function handleSaveUser() {
    ...
    if (userId === "new") {
      dispatch(addUser(getValues())).then(() => {
        navigate("/users");
      });
    } else {
      dispatch(updateUser(getValues())).then((res) => {
        ...
        <div className="flex flex-col items-center sm:items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
          ...
          >
            ...
            >
              ...
              <span className="flex mx-4 font-medium">Perfiles</span>
            </Typography>
          </motion.div>
