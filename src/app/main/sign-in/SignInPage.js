...
import { login, setUser, updateUserData } from "./../../store/userSlice";
...
function SignInPage() {
  ...
  function onSubmit({ email, password }) {
    ...
    // Simular un payload exitoso (fake user)
    const fakePayload = {
      id: "usuario.prueba",
      first_name: "Usuario",
      last_name: "Prueba",
    ...
    localStorage.setItem("data", encryptedObject.toString());

    // Despachar la acción `setUser` para guardar el usuario simulado en el estado
    dispatch(setUser(fakePayload));
    ...
    <div className="flex flex-col sm:flex-row items-center md:items-start sm:justify-center md:justify-start flex-1 min-w-0">
      <Paper className="h-full sm:h-auto md:flex md:items-center md:justify-end w-full sm:w-auto md:h-full md:w-1/2 py-8 px-16 sm:p-48 md:p-64 sm:rounded-2xl md:rounded-none sm:shadow md:shadow-none ltr:border-r-1 rtl:border-l-1">
        <div className="w-full max-w-320 sm:w-320 mx-auto sm:mx-0">
          ...
          <Typography className="mt-32 text-4xl font-extrabold tracking-tight leading-tight">Iniciar sesión</Typography>
          ...
          {openAlert ? <Alert severity="error">No se encuentra el usuario</Alert> : null}
        </div>
      </Paper>
