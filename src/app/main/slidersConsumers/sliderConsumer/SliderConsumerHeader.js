...
import { addSliderConsumer, updateSliderConsumer, removeSliderConsumer } from "../store/sliderConsumerSlice";
import { useDispatch } from "react-redux";
import { upLoadSingleImg } from "../../../utils";
import ConfirmationDialog from "app/shared-components/confirmationDialog";

function SliderConsumerHeader(props) {
  console.log('headerpropssssssssssssssss', props)
  ...
  function handleSaveSliderConsumer() {
    const data = getValues();
    data.type = type;
    if (sliderId === "new") {
      upLoadSingleImg(data, images).then(() => {
        dispatch(addSliderConsumer(data)).then(() => {
          navigate("/slidersConsumers");

        });
      });
    } else {
      upLoadSingleImg(data, images).then(() => {
        dispatch(updateSliderConsumer(data)).then((res) => {
  ...
  function handleRemoveSliderConsumer() {
    setOpenDialog(true);
  }

  function confirmRemove() {
    dispatch(removeSliderConsumer()).then(() => {
      ...
      <div className="flex flex-col sm:flex-row flex-1 w-full items-center justify-between space-y-8 sm:space-y-0 py-32 px-24 md:px-32">
        ...
        >
          <Button
            ...
            variant="contained"
            color="secondary"
            onClick={handleRemoveSliderConsumer}
            startIcon={
          ...
          <Button
            ...
            onClick={handleSaveSliderConsumer}
          >
            Guardar
          </Button>
        </motion.div>
...
export default SliderConsumerHeader;
