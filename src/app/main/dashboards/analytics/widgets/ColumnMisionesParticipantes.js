...
function ColumnMisionesParticipantes(props) {
  const dispatch = useDispatch();
  const widgets = useSelector(selectWidgets);
  const [awaitRender, setAwaitRender] = useState(true);
  const theme = useTheme();
  const { data, ranges } = widgets?.misiones_participantes ?? {
    data: [],
    ranges: {},
  };
  ...
  canalesUnicos.unshift("usuarios");
  ...
  const chartOptions = {
    ...
    legend: {
      customLegendItems: selectedChannel === "Mostrar Todos" ? ["activos", "no activos", "canales"] : [],
  ...
  useEffect(() => {
    ...
    const filteredData = () => {
      ...
      if (currentRange === "mes") {
        ...
        // Filtrar los datos para contar usuarios activos e inactivos del mes seleccionado
        const { activeUsers, inactiveUsers } = dataByDate.reduce(
          (counts, entry) => {
            entry.active ? counts.activeUsers++ : counts.inactiveUsers++;
            return counts;
          },
          { activeUsers: 0, inactiveUsers: 0 }
        );

        return [activeUsers, inactiveUsers, dataByDate];
      } else if (currentRange === "histórico") {
        dataByDate = data;

        // Obtener el inicio y el final del año actual en la zona horaria de Buenos Aires
        const startOfYear = currentDate.clone().startOf("year");
        const endOfYear = currentDate.clone().endOf("year");

        // Filtrar los datos para contar usuarios activos e inactivos del año en curso
        const { activeUsers, inactiveUsers } = data.reduce(
          (counts, entry) => {
            const fecha = moment(entry.fecha).tz("America/Argentina/Buenos_Aires");
            if (fecha >= startOfYear && fecha <= endOfYear) {
              entry.active ? counts.activeUsers++ : counts.inactiveUsers++;
            }
            return counts;
          },
    ...
    const [activeUsers, inactiveUsers, dataByDate] = filteredData();
    ...
    if (selectedChannel === "Mostrar Todos") {
      setSeries([
        { name: "Activos", data: [activeUsers], color: "#81bda4" },
        { name: "No activos", data: [inactiveUsers], color: "#4ed4e1" },
...
export default memo(ColumnMisionesParticipantes);
