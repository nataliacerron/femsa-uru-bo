...
import { updatePromotion, addPromotion } from "../store/promotionSlice"
...
function PromotionHeader(props) {
  ...
  function handleSavePromotion() {
    ...
    if (
      ...
      if (promotionId === "new") {
        upLoadImg(data).then(() => {
          dispatch(addPromotion(data)).then(() => {
            navigate("/promotions");
          });
        });
      } else {
        upLoadImg(data).then(() => {
          dispatch(updatePromotion(data)).then((res) => {
        ...
        <div className="flex flex-col items-center sm:items-start space-y-8 sm:space-y-0 w-full sm:max-w-full min-w-0">
          ...
          >
            ...
            >
              ...
              <span className="flex mx-4 font-medium">Promociones</span>
            </Typography>
          </motion.div>
        ...
        {promotionId !== "view" &&
          ...
          >
            ...
            <LoadingButton
              ...
              color="secondary"
              onClick={() => { handleSavePromotion(); setLoading(true); }}
              loading={loading}
              loadingPosition="start"
              startIcon={<SaveIcon />}
              variant="contained"
              disabled={
                promotionId === "new" ? _.isEmpty(dirtyFields) || !isValid : null
              }
            >
              <span>Guardar</span>
            </LoadingButton>
